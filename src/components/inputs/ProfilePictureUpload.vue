  ...
  <div class="profile-picture-upload">
    ...
    <div class="picture-container">
      <div class="picture-preview">
        <img v-if="currentPictureUrl && !isUploading" :src="currentPictureUrl" alt="Photo de profil" />
    ...
    <div class="actions">
      ...
      <button
        v-if="currentPictureUrl && !isUploading"
        type="button"
...
const emit = defineEmits<{
  ...
  (e: 'pictureChanged', mediaId: string, fileUrl: string): void;
  (e: 'pictureRemoved'): void;
}>();

const fileInput = ref<HTMLInputElement | null>(null);
const currentPictureUrl = ref<string | null>(props.modelValue);
...
watch(
  () => props.modelValue,
  (newValue) => {
    currentPictureUrl.value = newValue;
  }
);
  ...
  if (input.files && input.files.length > 0) {
    ...
    try {
      ...
      if (response?.data?.result) {
        ...
        currentPictureUrl.value = media.fileUrl;
        emit('update:modelValue', media.fileUrl);
        emit('pictureChanged', media.id, media.fileUrl);
      } else {
