(function (global, factory) {
  ...
  var BaseComponent = /*#__PURE__*/ (function () {
    ...
    _proto.dispose = function dispose() {
      ...
      for (
        var _iterator = _createForOfIteratorHelperLoose(Object.getOwnPropertyNames(this)), _step;
  ...
  var Marker = /*#__PURE__*/ (function (_BaseComponent) {
    ...
    _proto._createLabel = function _createLabel(_ref2) {
      ...
      this._offsets =
        isRecentlyCreated && marker.offsets ? marker.offsets : this.getLabelOffsets(index, label);
  ...
  function createMarkers(markers, isRecentlyCreated) {
    ...
    this._markerLabelsGroup =
      this._markerLabelsGroup || this.canvas.createGroup('jvm-markers-labels-group');
        ...
        switch (this._series.config.attribute) {
          ...
          case 'image':
            sample.style.background =
              'url(' +
              (typeof ticks[i].value === 'object' ? ticks[i].value.url : ticks[i].value) +
  ...
  function resize() {
    ...
    if (this._width / this._height > this._defaultWidth / this._defaultHeight) {
      ...
      this._baseTransX =
        Math.abs(this._width - this._defaultWidth * this._baseScale) / (2 * this._baseScale);
    } else {
      this._baseScale = this._width / this._defaultWidth;
      this._baseTransY =
        Math.abs(this._height - this._defaultHeight * this._baseScale) / (2 * this._baseScale);
    }
  ...
  function setScale(scale, anchorX, anchorY, isCentered, animate) {
    ...
    if (typeof anchorX != 'undefined' && typeof anchorY != 'undefined') {
      ...
      if (isCentered) {
        ...
        transY =
          anchorY + (this._defaultHeight * (this._height / (this._defaultHeight * scale))) / 2;
      } else {
  ...
  var DataVisualization = /*#__PURE__*/ (function () {
    ...
    _proto.getValue = function getValue(value) {
      ...
      for (var i = 0; i < 3; i++) {
        hex = Math.round(
          this._fromColor[i] +
            (this._toColor[i] - this._fromColor[i]) * ((value - this.min) / (this.max - this.min))
        ).toString(16);
    ...
    _proto._init = function _init() {
      ...
      if (options.series) {
        this.container.appendChild(
          (this.legendHorizontal = createElement('div', 'jvm-series-container jvm-series-h'))
        );
        this.container.appendChild(
          (this.legendVertical = createElement('div', 'jvm-series-container jvm-series-v'))
        );
        this._createSeries();
      }
